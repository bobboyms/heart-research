...
- **Denoise por mediana entre ciclos** (`--phase-contrast-cycle-median`: representa a sístole pela mediana do espectrograma entre ciclos; retreino do lb300, não fusão): AUPRC **0.8675** / AUROC 0.9397 vs lb300 0.8844/0.9375. **Não é ganho — é shift:** sobe Present (low-pitch 0.498→0.531, I/VI 0.588→0.627, FN 44→41) **E** Absent junto (0.081→0.093) → AUPRC −0.017. `29045` borrado (0.909→0.757, a mediana dilui sinal de ciclo-único). Confirma a hipótese do usuário **parcialmente** (ruído atrapalha o faint; limpar sobe os faint) mas **limpar desloca confiança, não adiciona separação**.
- **Quality-gate via SQI** (`scripts/sqi_quality_detector.py`): juiz de qualidade treinado no PhysioNet 2016 (rótulo SQI 0/1) — **CV AUROC 0.88** (válido). Transfere RELATIVAMENTE ao CirCor (corr 0.48 com confiança de segmentação TCN), mas scores absolutos sofrem domain shift (81% do CirCor cai <0.5, inutilizável como threshold absoluto). **Fusão tardia com lb300 = wash** (0.884→0.877): qualidade ⊥ presença (q_min AUROC 0.43; corr com prob lb300 = 0.10) e o `max` já ignora gravação ruim. **Detector é subproduto de QC; não melhora a detecção.** (6ª intervenção sem ganho.)
- **Conclusão (calibrada, bem suportada):** o teto do faint **não é falta de representação/features** — 6 intervenções independentes (energia, consistência, WST, transfer, denoise-ciclo, SQI-gate) não adicionam separação. É **informacional DADAS ESTAS GRAVAÇÕES + ESTA REPRESENTAÇÃO**. NÃO testado (única via plausível de info nova): representação **mais perceptual/aprendida** (SSL/foundational em PCG, que imite o "ouvido"); gravações melhores; dado pediátrico-rotulado-murmur. Não re-testar features de áudio sobre a STFT — esgotado.

## Interpretabilidade — ONDE está o piso (geometria de embeddings, `scripts/probe_embeddings.py`)
Extraídos os embeddings do encoder do lb300 (checkpoints por fold; sanity AUROC recomputado 0.907). **Os faint PERDIDOS estão embarcados em cima dos Absent no espaço aprendido**: distância ao centroide Absent — faint pegos 3.71, **faint perdidos 2.38 ≈ Absent 2.09**; um RF não os separa dos Absent (**AUROC 0.471 ≈ acaso**). → **Limite de REPRESENTAÇÃO, não do head** (cenário (a)): o encoder coloca os faint-perdidos indistinguíveis dos normais; o head está OK. Como o humano OUVIU esses sopros na gravação (rótulo vem da gravação — ver achado central), **o sinal existe mas a nossa representação não o contém** → reforça SSL/perceptual como única via, ou ruído de rótulo. **Lead acionável:** com segmentação GT o modelo pega **89/104 I/VI** (vs 64 com TCN) → **segmentação é gargalo real do faint** (oráculo; melhor TCN recuperaria ~25 faint, embora "mais épocas" não ajude — seria arquitetura/dados de TCN ou SSL).
